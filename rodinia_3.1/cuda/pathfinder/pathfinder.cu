...
init(int argc, char** argv)
{
	if(argc==4){
		...
		rows = atoi(argv[2]);
                ...
                //xin --printf("Usage: dynproc row_len col_len pyramid_height\n");
                exit(0);
        }
...
*/
    ...
    //printf("\n=== Pathfinder GPU Total Hardware Time ===\n");
    printf("GPU Time: %f ms\n", time_gpu_all);
    cudaEventDestroy(start_all);
    cudaEventDestroy(stop_all);
    //
    //xin: 内存占用打印（保留以便核对规模）
    unsigned long rows_ul = (unsigned long)rows;
    unsigned long cols_ul = (unsigned long)cols;
    unsigned long total_elements = rows_ul * cols_ul;
    unsigned long in_bytes = total_elements * sizeof(int);
    unsigned long out_bytes = cols_ul * sizeof(int);
    // Buffer 1 & 2: gpuResult[0] 和 gpuResult[1] (2 * cols)
    // Buffer 3: gpuWall (size - cols)
    // 总占用 = 2*cols + (size - cols) = size + cols
    unsigned long total_physical_bytes = (total_elements + cols_ul) * sizeof(int);
    double in_mb = (double)in_bytes / (1024.0 * 1024.0);
    double out_mb = (double)out_bytes / (1024.0 * 1024.0);
    double total_mb = (double)total_physical_bytes / (1024.0 * 1024.0);
    //printf("\n=== H100 Real Physical Consumption (PF) ===\n");
    //printf("Data IN (HtoD):       %.2f MB\n", in_mb);
    //printf("Data OUT (DtoH):      %.2f MB\n", out_mb);
