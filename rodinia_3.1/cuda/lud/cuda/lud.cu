...
main ( int argc, char *argv[] )
{
  ...
  if (input_file) {
    //xin-- printf("Reading matrix from file %s\n", input_file);
    ...
    //xin-- printf("Creating matrix internally size=%d\n", matrix_dim);
    ...
    //xin-- printf("No input file specified!\n");
    exit(EXIT_FAILURE);
  }

  if (do_verify){
    //xin-- printf("Before LUD\n");
  ...
  if (do_verify){
    //xin --printf("After LUD\n");
    //xin-- print_matrix(m, matrix_dim);
    //xin --printf(">>>Verify<<<<\n");
