...
{
  //xin-- printf("WG size of kernel 1 = %d, WG size of kernel 2= %d X %d\n", MAXBLOCKSIZE, BLOCK_SIZE_XY, BLOCK_SIZE_XY);
    int verbose = 1;
    int i, j;
    char flag;
    if (argc < 2) { /* xin--
        ...
        flag = argv[i][1];
          switch (flag) {
            case 's': // platform
              i++;
              Size = atoi(argv[i]);
	      //xin-- printf("Create matrix internally in parse, size = %d \n", Size);
    ...
    //xin-- printf("Time for CUDA kernels:\t%f sec\n",totalKernelTime * 1e-6);
...
void ForwardSub()
{
    ...
    //xin-- struct timeval time_start;
    //xin-- gettimeofday(&time_start, NULL);
	for (t=0; t<(Size-1); t++) {
	...
	// end timing kernels
//xin--    struct timeval time_end;
//xin--    gettimeofday(&time_end, NULL);
//xin--    totalKernelTime = (time_end.tv_sec * 1000000 + time_end.tv_usec) - (time_start.tv_sec * 1000000 + time_start.tv_usec);
    ...
    cudaEventElapsedTime(&time_gpu_all, start_total, stop_total);
    //printf("\n=== Gaussian GPU Total Hardware Time ===\n");
    printf("GPU Time: %f ms\n", time_gpu_all);
    cudaEventDestroy(start_total);
    cudaEventDestroy(stop_total);
    //
    // xin: 1. 内存占用计算 (Gaussian 涉及两个 Matrix 和一个 Vector)
    // 矩阵 a, m 大小均为 Size*Size，向量 b 大小为 Size
    unsigned long matrix_bytes = (unsigned long)Size * Size * sizeof(float);
    unsigned long vector_bytes = (unsigned long)Size * sizeof(float);
    unsigned long total_mem_allocated = matrix_bytes * 2 + vector_bytes;
    // HtoD 需要传输 a, m, b; DtoH 需要传回 a, m, b
    unsigned long h2d_bytes = total_mem_allocated;
    unsigned long d2h_bytes = total_mem_allocated;
    //printf("\n=== Gaussian Memory Analysis ===\n");
    //printf("Matrix Size: %d x %d\n", Size, Size);
    //printf("HtoD Transfer: %f MB\n", h2d_bytes / (1024.0 * 1024.0));
    //printf("DtoH Transfer: %f MB\n", d2h_bytes / (1024.0 * 1024.0));
    printf("GPU Memory: %.2f MB\n", total_mem_allocated / (1024.0 * 1024.0));
    //
}
