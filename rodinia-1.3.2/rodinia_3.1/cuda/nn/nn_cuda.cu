...
int main(int argc, char* argv[])
{
    ...
    */
    ...
    //cudaEventElapsedTime(&time_HtoD, start_htod, stop_htod);
    //
    /**
    * Execute kernel
    */
    ...
    //cudaEventElapsedTime(&time_DtoH, start_dtoh, stop_dtoh);
    //
    //xin
    // 统一打印汇总数据
        //printf("\n=== Figure 7 Performance Metrics (ms) ===\n");
        //printf("Init (Brown):       %f ms\n", time_Init);
        //printf("MemcpyHtoD (Blue):  %f ms\n", time_HtoD);
        //printf("Execution (White):  %f ms\n", time_Exec);
        //printf("MemcpyDtoH (Green): %f ms\n", time_DtoH);

        // 清理事件
        //cudaEventDestroy(start_init); cudaEventDestroy(stop_init);
        //cudaEventDestroy(start_htod); cudaEventDestroy(stop_htod);
        //cudaEventDestroy(start_exec); cudaEventDestroy(stop_exec);
        //cudaEventDestroy(start_dtoh); cudaEventDestroy(stop_dtoh);
        //
    //
    ...
    //float total_execution_time = (total_end.tv_sec - total_start.tv_sec) * 1000.0 + 
    //                             (total_end.tv_usec - total_start.tv_usec) / 1000.0;
    
    //printf("\nTotal Main Function Time: %f ms\n", total_execution_time);
    //
    //xin
    unsigned long h2d_bytes = (unsigned long)numRecords * sizeof(float2);
    unsigned long d2h_bytes = (unsigned long)numRecords * sizeof(float);
    double h2d_mb = h2d_bytes / (1024.0 * 1024.0);
    double d2h_mb = d2h_bytes / (1024.0 * 1024.0);
    double total_mb = h2d_mb + d2h_mb;
    printf("\n=== NN Memory Analysis ===\n");
    printf("DEBUG: Application Size = %d nodes\n", numRecords);
    //printf("DEBUG: HtoD Buffer Size = %f MB (%lu bytes)\n", h2d_mb, h2d_bytes);
    //printf("DEBUG: DtoH Buffer Size = %f MB (%lu bytes)\n", d2h_mb, d2h_bytes);
    printf("DEBUG: Total Memory Usage = %.2f MB\n", total_mb);
    //
}
