						...
						FOUR_VECTOR* fv_cpu)
{
    ...
    printf("\n=== lavaMD GPU Total Hardware Time ===\n");
    printf("Total GPU Time: %f ms\n", time_gpu_all);
    cudaEventDestroy(start_total);
    cudaEventDestroy(stop_total);
    //
    // xin: 内存占用计算
    // lavaMD 涉及四个主要 buffer：rv (位置), qv (电荷), fv (力), box (盒子数据)
    unsigned long total_particles = (unsigned long)dim_cpu.number_boxes * NUMBER_PAR_PER_BOX;
    unsigned long rv_bytes = total_particles * sizeof(FOUR_VECTOR);
    unsigned long qv_bytes = total_particles * sizeof(fp);
    unsigned long fv_bytes = total_particles * sizeof(FOUR_VECTOR);
    unsigned long box_bytes = (unsigned long)dim_cpu.number_boxes * sizeof(box_str);
    unsigned long total_mem = rv_bytes + qv_bytes + fv_bytes + box_bytes;
    printf("\n=== lavaMD Memory Analysis ===\n");
    printf("Number of Boxes: %d, Particles Per Box: %d\n", dim_cpu.number_boxes, NUMBER_PAR_PER_BOX);
    printf("Total Particles: %lu\n", total_particles);
    //printf("HtoD Transfer Size: %f MB\n", (rv_bytes + qv_bytes + box_bytes) / (1024.0 * 1024.0));
    //printf("DtoH Transfer Size: %f MB\n", fv_bytes / (1024.0 * 1024.0));
